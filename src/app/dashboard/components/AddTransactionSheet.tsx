            ...
            {assets.length > 0 && (
              <div className="space-y-1.5">
                <label htmlFor="account_id" className="block text-sm font-medium text-gray-700">
                  Account {type === "expense" && <span className="text-red-500">*</span>}
                  {type === "income" && <span className="text-gray-400 font-normal">(optional)</span>}
                </label>
                <select
                  id="account_id" name="account_id"
                  required={type === "expense"}
                  className="w-full px-3 py-2.5 rounded-xl border border-gray-300 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition"
                >
                  ...
                  {Object.entries(
                    ...
                    <optgroup key={cat} label={CATEGORY_LABELS[cat as AssetCategory] ?? cat}>
                      {group.map((a) => (
